...
export const Results: React.FunctionComponent<ResultsProps> = ({totalNumberOfPages, currentPage, cities, onNext, onPrevious}) => {
    if (cities.length === 0) {
        return <p data-testid={"empty-state"} className={"text-white pt-6"}>No cities available</p>;
    }

    return <div data-testid="result-list" className="w-full">
        <ol className={"m-6"}>
            {cities.map((c, i) => <CityListItem key={i} city={c}/>)}
        </ol>
